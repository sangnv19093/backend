  ...
  uploadAvatarMiddleware(req, res, async (err) => {
    try {
      if (err instanceof multer.MulterError) {
        ...
        return res.status(400).json({ error: 'Lỗi upload ảnh' });
      } else if (err) {
      ...
      console.log("id lấy:",userId);
      const avatarPath = path.join(uploadDir, req.file.filename);

      // Cập nhật đường dẫn ảnh trong cơ sở dữ liệu cho người dùng với userId
      const user = await userModel.userModel.findById(userId);

      if (!user) {
        console.error('Người dùng không tồn tại:', userId);
        return res.status(404).json({ error: 'Người dùng không tồn tại' });
      }

      // Lưu đường dẫn ảnh vào trường avatar của người dùng
      user.avatar = avatarPath;
      await user.save();

      console.log('Cập nhật ảnh đại diện thành công:', avatarPath);
      res.json({ avatarPath });
    } catch (error) {
...
exports.register = async (req, res, next) => {
  try {
    ...
    const existingUsername = await userModel.userModel.findOne({ username: req.body.username });
    if (existingUsername) {
      return res.status(501).json({ msg: 'Tên người dùng đã được sử dụng' });
    }
    // Kiểm tra sdt
    const existingPhone = await userModel.userModel.findOne({ phone: req.body.phone });
    if (existingPhone) {
    ...
    // Mọi thứ đều ổn, tiến hành tạo người dùng
